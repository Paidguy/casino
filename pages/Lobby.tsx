...
const GameCard = ({ title, to, icon, players, color, desc }: any) => (
  <Link 
    ...
    className="relative group overflow-hidden bg-bet-900 border border-white/5 p-8 rounded-[2.5rem] flex flex-col h-full transition-all hover:-translate-y-2 hover:border-bet-primary shadow-2xl active:scale-95"
  >
    ...
    <Layout>
      <div className="space-y-16 pb-20">
        ...
        <section className="relative overflow-hidden rounded-[3rem] lg:rounded-[4rem] bg-gradient-to-br from-bet-900 via-bet-950 to-black p-10 lg:p-20 shadow-3xl border border-white/5">
           ...
           <div className="relative z-10 space-y-8">
              <div className="inline-flex items-center gap-3 bg-white/5 px-5 py-2 rounded-full border border-white/10 backdrop-blur-xl">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-[0.4em] text-bet-primary">DEVELOPED BY @PAIDGUY • VERIFIED NODE</span>
              </div>
              <h1 className="text-5xl lg:text-[9rem] font-black text-white italic -skew-x-12 uppercase leading-[0.8] tracking-tighter bazar-font drop-shadow-3xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           {markets.map(res => (
             <div key={res.name} className="bg-bet-900/60 border border-white/5 p-10 rounded-[2.5rem] flex justify-between items-center group hover:border-bet-primary transition-all shadow-xl backdrop-blur-md relative overflow-hidden">
                <div className="absolute -left-10 -bottom-10 w-32 h-32 bg-white/5 rounded-full blur-3xl group-hover:bg-bet-primary/10 transition-colors"></div>
                <div className="relative z-10">
                   <p className="text-[10px] font-black text-slate-600 uppercase mb-3 tracking-widest">{res.name}</p>
                   <p className={`text-6xl font-black ${res.color} bazar-font tracking-[0.1em]`}>{res.result}</p>
                </div>
                <div className="text-right relative z-10">
                   <p className="text-[10px] font-black text-bet-accent uppercase mb-3 tracking-widest">{res.type}</p>
        ...
        <section className="space-y-12">
           <div className="flex items-center justify-between px-4">
              <h2 className="text-4xl lg:text-7xl font-black text-white italic -skew-x-6 uppercase tracking-tighter bazar-font leading-none">Global <span className="text-bet-primary">Bazar</span></h2>
        ...
        <footer className="pt-20 pb-10 border-t border-white/5 text-center">
            <div className="text-[12px] font-black text-slate-700 uppercase tracking-[0.6em] mb-4">Masterfully Crafted By</div>
            <div className="text-4xl lg:text-6xl font-black text-white italic -skew-x-12 uppercase bazar-font drop-shadow-2xl">@PAID<span className="text-bet-primary">GUY</span></div>
            <div className="text-[9px] text-slate-800 font-bold uppercase tracking-widest mt-8">SattaKing.IND Pro v8.0 Final Build • Hyper-Silk Logic Engine</div>
        </footer>
      </div>
    </Layout>
  );
